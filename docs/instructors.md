...
- Amazon Lookout for Metrics: uses ML to detect anomalies in virtually any timeseries-driven business and operational metrics (revenue performance, purchase transactions, customer acquisitions, retention rates). Ex: Category-level Revenue in Retail, Ad Tech company can optimize ad spend by detecting spikes/dips in metrics like reach, impressions, views, ad clicks. You can automatically trigger a custom Lambda function whenever an anomaly is detected. Ex: Pull the product off the site until further inspection is done. It uses your feedback to continuously optimize its algorithm and improve its accuracy over time. You can visualize and review the details of these anomalies in AWS console or retrieve them through an API. Dr. Matt Wood: iHeartMedia uses this service for real-time music recommendations; Lyft is gathering petabytes of data and anlyzing it with SageMaker to improve self-driving systems; JP Morgan for personalization to its client interactions like real-time coaching and recommendations for contact center agents for better customer service.  
- Amazon Monitron: end-to-end solution for industrial equipment monitoring. It includes Sensors -> Network Gateway Device -> Mobile App. The sensors measure temperature, vibration in 3 dierctions when mounted on equipment like motors, gear boxes, compressors, turbines, fans, pumps, etc. You get push notifications to mobile app about potential anomalies.  
- Amazon Lookout for Equipment: anomaly detection service for industrial machinery. Send the data to AWS (gets stored in S3) -> Analyzes data from upto 300 sensors per industrial machinery -> Pinpoints individual sensors indicating anomalies -> Sends the anomalies via an API for Predictive Maintenance -> Integrated with existing monitoring software such as IoT Sitewise or industrial data systems such as OSISoft -> Enables automatic actions when anomalies are detected.  
